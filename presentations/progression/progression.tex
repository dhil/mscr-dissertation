...
\begin{itemize}
  \item \lstinline$Spawn : (() \{ |e\}-> ()) -> Process(\{ |e\})$
  \item \lstinline$Yield : ()$
  \item \lstinline$Self : Process(\{ |e\})$
\end{itemize}
\vfill
Operations for communication
\begin{itemize}
  \item \lstinline$Send : (m, Process(\{ |e\})) -> ()$
...
\begin{frame}
  ...
  Roundrobin scheduler. Basic idea: enqueue and dequeue continuations.
  Modular implementation.
  Global state to manage mailboxes.
\end{frame}

%% Intuition behind the sieve example
\begin{frame}
\frametitle{The intuition behind the sieve example}
\begin{figure}
