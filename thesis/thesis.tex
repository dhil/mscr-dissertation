...
\end{itemize}
The process queue is naturally stateful as it will change every time a
process is being spawned or suspended. Therefore we will use a state
interpretation The auxiliary functions \lstinline$enqueueProcess$ and
\lstinline$dequeueProcess$ hides the details of the process queue
management. We do not present the implementation details of the
underlying queue data structure. In order to maintain the queue we
require a stateful interpretation. The two function use the
\lstinline$Get$ and \lstinline$Put$ operations that we defined in
Section~\ref{sec:??}:
%
...
%
There are two obvious scheduling policies to choose from
