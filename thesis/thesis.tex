...
\lstdefinelanguage{Links}{% 
  ...
  emph={Comp,Player,Bool,Int,GTree,Cheat,Zero,Choose,Rand,Move,Winner,Take,Return,Get,Put,GameState,Alice,Bob,Fail,Nothing,Just,Maybe,Toss,Heads,Tails,Process,Buyer,Coffee,Pay,Cost,Candidate,Stop,PassingComet,CelebritySighting,Float,String},
...
the interaction between two processes. The process $P_0$ cannot access
the state of process $P_1$ -- and vice versa. Each mailbox is
intrinsic to each process. The process $P_0$ interact with $P_1$ via a
message $s$ depending on the contents of $s$ the process $P_1$ may
...
\tikzset{sarrow/.style={-latex,thick,dotted}}
  ...
  \node(P0) [draw=none,minimum width=30pt]         {$P_0$};
  ...
  \node(P0M) [fit={(P0M0) (P0M1) (P0MC) (P0MN) (P0MT)}] { };
  \node(P0B) [fit={(P0M) (P0)}] { };

  \node(P1) [draw=none,below=of P0,yshift=-70pt,minimum width=30pt]         {$P_1$};
  ...
  \node(P1M) [fit={(P1M0) (P1M1) (P1MC) (P1MN) (P1MT)}] { };
  \node(P1B) [fit={(P1M) (P1)}] { };
...
\draw[sarrow] (P0B.south east) to [out=300, in=120,style={-latex,thick,dotted}]  (P1B.north west) node [draw=none,midway,yshift=-35pt,xshift=-80pt] {$P_0$ sends message $s$ to $P_1$};
\end{tikzpicture}
...
We shall implement a parallel version of the \emph{Sieve of
  Eratosthenes} prime number finding algorithm. Starting from a prime
...
\node(P0)[my ellipse] {$P_0$ (Generator)};

\node(P1)[my ellipse,below=of P0,yshift=-20pt] {$P_1^2$};
\node(P2)[my ellipse,right=of P1,xshift=85pt] {$P_2^3$};
\node(P3)[my ellipse,below=of P2,yshift=-20pt] {$P_3^5$};
\node(P4)[my ellipse,left=of P3,xshift=-85pt] {$P_4^7$};
...
\citep{Andrews2000}. Figure~\ref{fig:sieve} depicts the sieve pipeline
which finds primes between $2$ and $10$. The job of each sieve process
is to receive and perform a primality test on candidate prime by
dividing the candidate number by its own prime. If the remainder after
division is positive then the process forwards the candidate to its
neighbour process. The initial process generates a sequence of natural
numbers. These numbers are sent one by one to the first sieve
process. We implement this as the function \lstinline$generator(n)$
where \lstinline$n$ is the upper bound on sequence being generated:
%
\begin{lstlisting}
fun generator(n) {
...
\end{lstlisting}
%
The function spawns the first sieve process and afterwards it
generates the sequence $\{2,\dots,n\}$. The \lstinline$foreach$
function has type \lstinline$([a], (a) ~e~> ()) ~e~> ()$, that is it
takes a list and an action as arguments. The action is applied to each
element in the list.
%
\begin{lstlisting}
fun sieve() {
