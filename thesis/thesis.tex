...
\abstract{%
  %
  Algebraic effects combined with effect handlers have emerged as
  compelling, modular abstraction for modelling and controlling
  computational effects. By separating the effect signatures from
  their implementation, algebraic effects afford a high degree of
  modularity as programmers can describe effectful computations
  independently of their concrete interpretation.
  
  We present a compiler for the functional programming language Links
  that uses the Multicore OCaml backend to provide a native
  ...
  a subset of the intermediate language used by OCaml.

  Interestingly, concurrency can be described as an algebraic effect,
  whose handler amounts to a scheduler. Thus, rather than baking
  concurrency support into the compiler, we keep the compiler lean by
  implementing the message-passing concurrency model of Links using
  handlers. We demonstrate a faithful encoding of the concurrency
  model, which maintains type-safe communication by taking advantage
  of the effect system of Links.

  Finally, we perform some experiments with the compiler using the
  concurrency implementation to see how it performs against the Links
  interpreter. Moreover, we consider how to improve the performance of
  the compiler.
%   \todo{Refine abstract} 
% %
...
%, taking advantage of
%the effect type system and the subformula property to guarantee query
%generation~\citep{Lindley2012}.
%
...
% exceptions, file i/o, concurrency, nondeterminism, and so forth. Many
% programming languages provide no or little support for controlling
% computational effects. Recently \emph{algebraic
%   effects}~\citep{Plotkin2001,Plotkin2003} and combined with
% \emph{handlers}~\citep{Plotkin2013} have emerged as a modular approach
% for managing user-defined computational effects. An effect is
% described as a signature over a collection of \emph{abstract
%   operations} whose implementations are given by one or more
% handlers. The main idea of programming with algebraic effects and
% handlers is that performing an effectful operation is separate from
% its interpretation. 

% Effect handlers generalise exception handlers, providing a mechanism
% for interpreting arbitrary algebraic effects. Crucially, programming
% with handlers relies on non-linear control. When an effectful
% computation performs an effect it gets suspended and control is
% transferred to some suitable handler -- much like throwing an
% exception in any conventional programming language. However, unlike
% exception handlers, effect handlers expose an interface that permits
% resumption of the suspended computation.

% %Consequently, effect handlers afford a high degree of modularity.

% Since \cite{Plotkin2013} conceived handlers many different
% implementations have appeared
% \citep{Brady2013,Kammar2013,Kiselyov2013,Kiselyov2015,Bauer2015,Dolan2015,Lindley2016,McBride2016,Saleh2016}.
% Most of these implementations try to embed effect handlers within an
% existing programming language. Interestingly, \cite{Dolan2015} have
% extended the industrial-strength functional programming language OCaml
% with particular, restricted kind of handlers known as \emph{affine
%   handlers} which permit suspended computations to be resumed at most
% once. Their implementation is part of the Multicore OCaml project to
% bring multicore capabilities to the OCaml programming language. They
% adopt the idea of \cite{Bauer2015} to use effect handlers to implement
% user-defined concurrent multi-threaded schedulers.
% %

% In previous work, we augmented the functional programming language
% Links with algebraic effects and handlers
% \citep{Hillerstrom2015}. Links is a strict, single-source, interpreted
% programming language for multi-tier web systems. Additionally, it has
% a built-in type-safe distributed message-passing concurrency model. We
% are interested in further extending Links with a native compiler for
% effect handlers. In the long term we are interested in studying the
% implementation space of effect handlers. 
...
%Benefits: Simplifies the life of the compiler writer; provides the
%programmer with more flexibility.
...
%
\lstinputlisting{snippets/getput.links}
%
...
%
\lstinputlisting{snippets/evalState.links}
%
...
%
We can send a message to process \lstinline$1$ using the \lstinline
$!$ primitive (pronounced ``send''):
%
\begin{lstlisting}
links> p2 ! "Hello";
Message: Hello
...
of received message. As a concrete example consider this example
adapted from the Links documentation \citep{Links} where a process
that gets informed about passing comets and celebrity sightings:
\begin{lstlisting}
var p3 = spawn {
  fun loop() {
    var _ = switch(recv()) {
...
3 : Process ({ hear:[|CelebritySighting:(String, String)
             ...
             , wild|_ })
\end{lstlisting}
We can inform the process of any passing comets and celebrity
sightings:
%
\begin{lstlisting}
...
example to demonstrate our reconstruction of the concurrency model in
Chapter~\ref{ch:concurrency}.
...
\begin{description}
\item[Line 2] receives the process' prime number. The
  \lstinline$fromCandidate$ simply removes the \lstinline$Candidate$
  tag from the number.
\item[Line 3] simply prints the prime to standard out.
\item[Line 4] begins the definition of the main loop of the
  process. The function is parameterised by a handle to its
...
\item[Lines 5-9] dispatch on the message tag. If the message is
  ...
  number is composite the \lstinline$loop$ function recurse in order
...
\item Frank by \cite{Lindley2016} takes the idea of effect handlers to
  ...
  % Frank's effect system can be viewed as implementing a
...
\item Shonky by \cite{McBride2016} amounts to a dynamically-typed
  ...
  % does not use an ANF representation, so has more
  % forms of continuation to handle, and in contrast to our nested
  % continuation structure, Shonky uses a completely flat structure.
...
% % By separating the signature of an applicative from its
% % implementation
...
\caption{Kinding rules \citep{Hillerstrom2016a}.}
\label{fig:kinding}
\end{figure}
...
\caption{Typing rules \citep{Hillerstrom2016a}.}
\label{fig:typing}
\end{figure*}
...
\inferrule*[Lab=\semlab{Handle-Op}]
  ...
  { \Handle \; \mathcal{E}[\Do \; \ell \; V] \; \With \; H
     \reducesto N[V/x, \lambda y . \, \Handle \; \mathcal{E}[\Return \; y] \; \With \; H/k],\\
...
\begin{mathpar}
\begin{eqs}
  \htrans{\{\Return\; x \mapsto M\} \uplus H} &=& \Alloc \left(
 \begin{eqs}
   &&\Fun \; \vtrans{x} \mapsto \mtrans{M},\\
   &&\Error,\\
   &&\Fun \;z \mapsto \Let\; y \revto \Field_1\; z \; \In \\
   &&\Let\; k \revto \Field_2\; z \; \In \; \hptrans{H,y,k}
 \end{eqs}
\right)
\end{eqs}
\end{mathpar}
...
%  \htrans{H} &=& \Fun \; x \mapsto \Fun \; k \mapsto \hptrans{H}^{x,k}\\
  \hptrans{\{\ell \, x \, k \mapsto M\} \uplus H, \LBox{l; x'}, k'} &=&  \If \; \Eq(\mathcal{N}(\ell), l) \; \Then \\
                                                       & & 
\begin{eqs}
...
\begin{mathpar}
\begin{eqs}
  \vtrans{(\ell \; V)^E} &=& \LBox{\mathcal{N}(\ell); \vtrans{V}}
\end{eqs}
\end{mathpar}
...
% However, cloning is a fragile abstraction; if the handler stack
% contains at least one multi-shot handler, then every affine handler
% in the stack must be demoted to a multi-shot handler to be safe,
% because a multi-shot handler may consume a linear continuation more
% than once. Consequently, multi-shot handlers in OCaml break
% modularity.
...
% identifier of the executing process. However, we will soon realise
...
\cline{2-6}
 \multicolumn{1}{c|}{~} & \multicolumn{2}{c |}{\textbf{Links interpreter}} & \multicolumn{2}{c |}{\textbf{Links compiler}} & \multicolumn{1}{c |}{\textbf{ocamlopt}} \\
\hline
...
% \hline
% Board size  & 8 & 12 & 16 & 20 \\
% \hline
%   Interpreter/no handlers & 0.0 & 0.0 & 0.0 & 0.0\\
% \hline
%   Links compiler    & 0.0 & 0.0 & 0.0 & 0.0\\
% \hline
% \end{tabular}
...
% In the concurrency implementation all mailboxes are homogeneous, that
% is, they can only contain messages of a single type. It would be
% beneficial to permit mailboxes to be heterogeneous. This would allow 
