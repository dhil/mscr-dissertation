...
\paragraph{Perform} The $\Perform(L)$ primitive invokes an abstract
operation. The behaviour of $\Perform$ is only defined when $L$
evaluates to a name of an abstract operation.

\paragraph{Resume} The $\Resume(S,L)$ primitive evaluates the
expression $L$ with a handler $S$. If an abstract operation is
performed in $L$ then the run-time attempts to find a suitable
operation clause in the handler $S$.

\paragraph{Clone} The $\Clone(L)$ primitive (shallowly) copies an
expression $L$.

\paragraph{Delegate} The $\Delegate(L,J)$ primitive forwards an
abstract operation name $L$ along with the continuation, $J$, of the
abstract operation to another enclosing $\Resume$-block.

\paragraph{Alloc\_stack} In Lambda handlers are represented as
stacks. The $\Alloc(L,I,J)$ primitive allocates a new handler with
return clause $L$, an exception clause $I$, and an operation clause
$J$.
