...
\end{lstlisting}
As usual the operation is quite simple. Most of the work is concerned
...
semantics. There are two obvious scheduling policies to choose from
when a process invokes the \lstinline$Spawn$ operation. The scheduler
either enqueues the parent process and runs the child process
immediately or enqueues the child process and resumes the parent
process. We will adopt the former policy. Using this policy an
...
\item[Lines 2-5] handle the \lstinline$Spawn$ operation by first
  ...
  \lstinline[mathescape]!EProcess($\cdot$)! to the new
  \lstinline$child$ process. This handle is returned to the parent via
  the resumption function, \lstinline$resume$. However, the evaluation
  of the resumption function is delayed as we store it inside a thunk.
  This effectively amounts to suspending the process. The thunk goes
  into the process queue. Afterwards the scheduler transfers control
  to the forked computation \lstinline$f$. It is crucial that the
  scheduler is invoked recursively in order to handle any effects that
...
\end{description}
%
The auxiliary functions \lstinline$enqueueProcess$ and
\lstinline$dequeueProcess$ hides the details of the process queue
management. In order to maintain the queue we require a stateful
interpretation. The two function uses the \lstinline$Get$ and
\lstinline$Put$ operations that we define in Section~\ref{sec:??}:
%
\begin{lstlisting}
sig enqueueProcess : 
   (() -e-> ()) {Get: Queue(() -e-> ())
                ,Put:(Queue(() -e-> ())) {}-> ()|_}~> ()
fun enqueueProcess(p) {
  var q = enqueue(get(), p);
  put(q)
}
\end{lstlisting}
Here \lstinline[mathescape]!Queue($\cdot$)! is a type constructor for
a queue type. We will omit the implementation details of the queue
itself. The function \lstinline$enqueue$ stores the process
\lstinline$p$ in the queue retrieved by the \lstinline$get$
operation. Similarly, \lstinline$dequeueProcess$ removes a process
from the queue. However when the queue is empty it returns the trivial
process. This way we will not have to concern ourselves with special
cases in the scheduler. The implementation is straightforward:
%
\begin{lstlisting}
fun dequeueProcess() {
  var q = get();
  if (isEmptyQueue(q)) { fun() { () } } # Special case
  else {
    var (p, q) = dequeue(q);
    put(q);
    p # Returning the process
  }
}
\end{lstlisting}
%
