...
accommodate for the special case where a mailbox is empty:
%
...
We need to make a few changes to sieve example in order to adapt it to
our new concurrency implementation. The changes are only
syntactical. For example, we changes the \lstinline$generator$
function as follows:
%
...
7
\end{lstlisting}
The resulting program yields the same output as the program we
