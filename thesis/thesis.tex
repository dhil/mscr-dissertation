...
In (purely) functional programming monads have proven to be remarkable
successful abstraction for handling computational effects despite
their lack of modularity \citep{Kammar2013,Kiselyov2013}. Recently,
\emph{algebraic effects} \citep{Plotkin2001,Plotkin2003} and their
\emph{handlers} \citep{Plotkin2013} have emerged as a modular
alternative to handling computational effects.

Since the inception of effect handlers many different implementations
have appeared.
%
\todo{Fix introduction}
%
...
The squiggly function arrow (\lstinline$~>$) is syntactic sugar for
denoting that the computation has the \emph{wild} effect. The wild
effect captures all intrinsic effects such as I/O, randomness,
divergence, etc. To some extent it is analogous to the \emph{IO monad}
of Haskell, though the wild effect is much stricter as without it
general recursion is disallowed.
\todo{Describe the effect system}
