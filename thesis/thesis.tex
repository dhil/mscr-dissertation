...
semantics. In order to maintain the queue we will use a stateful
interpretation. We will define two auxiliary functions to enqueue and
dequeue processes from the queue:
%
\begin{lstlisting}
sig enqueueProcess : 
   (() -e-> ()) {Get:Queue(() -e-> ())
...
process. We will adopt the former policy. When \lstinline$Yield$ is
invoked We implement this scheduler as a handler:
