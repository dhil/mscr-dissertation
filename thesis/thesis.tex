...
\lstdefinelanguage{Links}{% 
  ...
  emph={Comp,Player,Bool,Int,GTree,Cheat,Zero,Choose,Rand,Move,Winner,Take,Return,Get,Put,GameState,Alice,Bob,Fail,Nothing,Just,Maybe,Toss,Heads,Tails,Process,Buyer,Coffee,Pay,Cost,Candidate,Stop,PassingComet,CelebritySighting,Float,String,Pid,EProcess,Row},
...
\end{lstlisting}
%
If it has no neighbour it silently exists, otherwise it forwards the
\lstinline$Stop$ message before exiting. Now, we can run the example:
\begin{lstlisting}
links> generator(10);
2
3
...
links> makeProcess(fun() { if (do Choose) { print("True") } else { print("False") } }, 2);
(id=2) : EProcess ({ |Choose:Bool,wild|_ })
\end{lstlisting}
%

\subsection{A round-robin scheduler}
